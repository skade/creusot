...
impl Type {
        ...
        A::Doc: Clone,
    {
        ...
        match self {
            ...
            TFun(box a, box b) => ty_parens!(alloc, env, a)
                .append(" -> ")
                .append(ty_parens!(alloc, env, b)),
            TApp(box tyf, args) => {
                if args.is_empty() {
...
impl Exp {
        ...
        A::Doc: Clone,
    {
        match self {
            ...
            Exp::Tuple(args) => alloc.intersperse(args.iter().map(|a| a.pretty(alloc, env)), ", ").parens(),

            Exp::Constructor { ctor, args } => ctor.pretty(alloc, env).append(if args.is_empty() {
                alloc.nil()
            } else {
                alloc
                    .intersperse(args.iter().map(|a| a.pretty(alloc, env)), ", ")
                    .parens()
            }),
        ...
        A::Doc: Clone,
    {
        match self {
            Statement::Assign { lhs, rhs } => {
                lhs.pretty(alloc, env).append(" <- ").append(parens!(alloc, env, Precedence::Assign, rhs))
            }
            ...
            Statement::Assume(assump) => alloc.text("assume ").append(
                alloc.space().append(assump.pretty(alloc, env)).append(alloc.space()).braces(),
            ),
            Statement::Assert(assert) => alloc.text("assert ").append(
